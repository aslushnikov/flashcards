...
module.exports = function(app, actions, config) {
    ...
    app.post("/import", ensureLogin, function(req, res, next) {
        ...
        if (req.body.csv.substr(0, prefix.length) !== prefix)
            return res.send(400);
        var csv = new Buffer(req.body.csv.substring(prefix.length), "base64").toString("utf8");
        actions.importUserWords(req.user, csv)
        .then(function() {
            res.send(200);
        })
        .fail(function() {
