...
Actions.prototype = {
    ...
    dismissUserNotification: function(user, notificationId)
    {
        var Notification = this._db.models.notification;
        return Q.denodeify(Notification.get.bind(Notification))(notificationId)
        .then(function(notification) {
            if (user.email !== notification.to) throw new Error("User with email " + user.email + " cannot dismiss notification which belongs to user with email " + notification.email);
            return Q.denodeify(notification.remove.bind(notification));
        })
    },

    dismissAllUserNotifications: function(user)
    {
        var Notification = this._db.models.notification;
        var deferred = Q.defer();
        Notification.find({to: user.email}).remove(function(err) {
